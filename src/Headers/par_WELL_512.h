/*
    Copyright 2013-2018 Renwick James Hudspith

    This file (par_WELL_512.h) is part of GLU.

    GLU is free software: you can redistribute it and/or modify
    it under the terms of the GNU General Public License as published by
    the Free Software Foundation, either version 3 of the License, or
    (at your option) any later version.

    GLU is distributed in the hope that it will be useful,
    but WITHOUT ANY WARRANTY; without even the implied warranty of
    MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
    GNU General Public License for more details.

    You should have received a copy of the GNU General Public License
    along with GLU.  If not, see <http://www.gnu.org/licenses/>.
*/
/**
   @file par_WELL_512.h
   @brief prototype declarations for the WELL 512
 */
#ifndef GLU_PAR_WELL_512_H
#define GLU_PAR_WELL_512_H

/**
   @fn void free_par_WELL_512( void )
   @brief free the RNG table
 */
void
free_par_WELL_512( void ) ;

/**
   @fn void GLU_set_par_WELL_512_table( const uint32_t seed[ Latt.Nthreads ] ) 
   @brief set the parallel table
   @param seed :: seeds for the RNG
 */
void
GLU_set_par_WELL_512_table( const uint32_t seed[ Latt.Nthreads ] ) ;

/**
   @fn double par_WELL_512_dbl( const uint32_t thread )
   @brief create a double precision random number
   @param thread :: parallel thread index
 */
double 
par_WELL_512_dbl( const uint32_t thread ) ;

/**
   @fn int read_par_WELL_512_table( FILE *rng_file )
   @brief read the Well RNG table from rng_file
   @param rng_file :: file to read the table from
   @return #GLU_SUCCSS or #GLU_FAILURE
 */
int
read_par_WELL_512_table( FILE *rng_file ) ;

/**
   @fn void write_par_WELL_512_table( FILE *rng_file )
   @brief write the WELL RNG table to rng_file
   @param rng_file :: file to write out the table
 */
void
write_par_WELL_512_table( FILE *rng_file ) ;

#endif
